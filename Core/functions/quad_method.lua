...
--local quads = {};
function Check_Quad_Region(search_section)
    		...
    		for x = -1, 1 do
    			...
    			( y == 0 and (x == -1 or x == 1)) then
                    ...
                    if (return_info[2] == -1) then
                        --print("no card in region x" .. x .. " y" .. y)
                         print("no card, " .. direction)
                    else
                        local card_info = retrieve_card(GameInfo.quads[return_info[2]].filename)
                        --print("list pos:" .. return_info[2] .. " card:" .. card_info.name)
